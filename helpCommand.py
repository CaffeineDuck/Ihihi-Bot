...
class CustomBotHelpCommand(commands.HelpCommand):
    ...
    def subcommand_not_found(self,command: commands.Command, string: str) -> str:
        return f'I don\'t have the command `{command.qualified_name} {string}`, if you have an idea for this suggest it in our support server! [here](https://discord.gg/t2M5mCW6Ed)'
    ...
    async def send_bot_help(self, mapping: Mapping[Optional[commands.Cog], List[commands.Command]]) -> None:
        ...
        for cog in self.context.bot.cogs.values():
            help_string = cog.__doc__ if cog.__doc__ is not None else 'No help available'
            embed.add_field(name=cog.qualified_name, value=f'{help_string}\n`{self.clean_prefix}help {cog.qualified_name}`')
        await self.dispatch_help(embed)
    ...
    async def send_command_help(self, command: commands.Command) -> None:
        ...
        embed.add_field(name='What does this command do?', value=command.help, inline=False)
    ...
    async def send_cog_help(self, cog: commands.Cog) -> None:
        ...
        embed.add_field(name='What does this extension do?', value=cog.__doc__, inline=False)
